...
| Student | Check 1 | Check 2 | Check 3 | Total | Status  |
|---------|---------|---------|---------|-------|---------|
| Alice   | ✅      | ✅      | ✅      | 3/3   | PRESENT |
| Bob     | ✅      | ❌      | ❌      | 1/3   | LATE    |
| Charlie | ❌      | ❌      | ❌      | 0/3   | ABSENT  |

- **PRESENT** (green): detected in ≥ 2 checks
- **LATE** (yellow): detected in exactly 1 check
- **ABSENT** (red): detected in 0 checks
...
5. **Final computation**: ≥ 2 checks = present, 1 = late, 0 = absent.
...
| `SIMILARITY_THRESHOLD`   | `0.6`       | Cosine similarity cutoff for matching    |
| `IMAGES_PER_REGISTRATION`| `5`         | Faces to capture per student             |
| `MIN_CHECKS_FOR_PRESENT` | `2`         | Minimum checks for "present" status      |
