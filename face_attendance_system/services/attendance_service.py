...
class AttendanceMonitor:
    ...
    def __init__(self, face_detector, face_embedder, face_database, attendance_store, twin_handler=None):
        ...
        self._latest_spoofed = []  # indices of spoofed faces in latest frame
        ...
        self._latest_spoofed = []  # indices of spoofed detections
    ...
    def start_session(self, class_name, camera_index=None):
    ...
    def _execute_check(self, session_id, check_number):
        ...
        for fi in range(total_frames):
            ...
            if detections:
                ...
                matches = self.matcher.find_all_matches(embeddings)
                names = []
                spoofed_indices = []
                ...
                for idx, m in enumerate(matches):
                    if m:
                        ...
                        if face_crop is not None and Config.LIVENESS_ENABLED:
                            liveness = self.liveness_detector.quick_liveness_check(face_crop)
                            if not liveness["is_live"]:
                                # SPOOFING DETECTED — skip this face
    ...
    def _update_live_preview(self):
        ...
        if detections:
            embeddings = [self.embedder.get_embedding(d["cropped_face"]) for d in detections]
            matches_raw = self.matcher.find_all_matches(embeddings)
    ...
    def _compute_final(self, session_id):
        ...
        for sid, r in final.items():
            ...
            if sid in uncertain_sids and r["status"] in ("present", "late"):
                r["needs_review"] = True
                if not r.get("note"):
