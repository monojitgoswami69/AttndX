...
class LivenessDetector:
    ...
    def __init__(self):
        ...
        # Deep-learning model-based spoof detector (primary method).
    ...
    def quick_liveness_check(self, face_crop, frame=None, face_bbox=None):
        """
        Fast single-frame liveness check.

        Uses the deep-learning MiniFASNetV2 model as the primary method
        (trained on real spoofing data — far more reliable than heuristics).
        The model needs the FULL FRAME + face bbox to crop with context
        (surrounding area helps detect phone/screen edges).
        Falls back to texture + frequency analysis on the tight face crop if
        the model or frame is unavailable.
        ...
        # ── Primary: deep-learning model (needs full frame + bbox for context) ──
        if (
            self.spoof_detector is not None
            and self.spoof_detector.is_available()
            and frame is not None
            and face_bbox is not None
        ):
            result = self.spoof_detector.check(frame, face_bbox)
            if result is not None:
                return result

        # ── Fallback: heuristic texture + frequency on the tight crop ──
