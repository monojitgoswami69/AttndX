...
class TwinHandler:
    ...
    def _cosine_similarity(self, a, b):
        """Cosine similarity (= dot product for L2-normalized embeddings)."""
        a = a.flatten()
        b = b.flatten()
    ...
    def _avg_similarity(self, embeddings_a, embeddings_b):
        """Compute average of best pairwise similarities (vectorized)."""
        if not embeddings_a or not embeddings_b:
            return 0.0
        mat_a = np.stack([e.flatten() for e in embeddings_a])  # (N, D)
        mat_b = np.stack([e.flatten() for e in embeddings_b])  # (M, D)
        na = np.linalg.norm(mat_a, axis=1, keepdims=True)  # (N, 1)
        nb = np.linalg.norm(mat_b, axis=1, keepdims=True)  # (M, 1)
        safe_a = np.where(na < 1e-10, 1.0, na)
        safe_b = np.where(nb < 1e-10, 1.0, nb)
        sim_matrix = (mat_a @ mat_b.T) / (safe_a * safe_b.T)  # (N, M)
        best_per_a = np.max(sim_matrix, axis=1)  # (N,)
        return float(np.mean(best_per_a))

    def _max_pair_similarity(self, embeddings_a, embeddings_b):
        """Compute the single highest pairwise cosine similarity (vectorized)."""
        if not embeddings_a or not embeddings_b:
            return 0.0
        mat_a = np.stack([e.flatten() for e in embeddings_a])
        mat_b = np.stack([e.flatten() for e in embeddings_b])
        na = np.linalg.norm(mat_a, axis=1, keepdims=True)
        nb = np.linalg.norm(mat_b, axis=1, keepdims=True)
        safe_a = np.where(na < 1e-10, 1.0, na)
        safe_b = np.where(nb < 1e-10, 1.0, nb)
        sim_matrix = (mat_a @ mat_b.T) / (safe_a * safe_b.T)
    ...
    def check_for_twins(self, new_student_id, new_embeddings, face_database):
        ...
        for sid, data in students.items():
            ...
            pair_sim = self._max_pair_similarity(new_embeddings, existing_embs)
            if sim > best_sim:
                best_sim = sim
                best_pair_sim = pair_sim
                best_sid = sid
                best_name = data.get("name", sid)

        result["max_similarity"] = round(best_sim, 4)
        result["max_pair_similarity"] = round(best_pair_sim, 4)
