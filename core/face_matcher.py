...
class FaceMatcher:
    ...
    def _cosine_similarity(self, a, b):
        ...
        Falls back to the general formula if vectors are not normalized.
        """
        a = a.flatten()
        b = b.flatten()
        dot = float(np.dot(a, b))
        # Since embeddings are L2-normalized at generation time, dot ≈
        # cosine similarity. Guard against un-normalized edge cases.
    ...
    def _best_similarity(self, query, stored_embeddings):
        """Best cosine similarity between a query and a list of stored
        embeddings. Vectorized: stacks stored embeddings into a matrix and
        does one matrix-vector product (much faster than a Python loop for
        many embeddings per student)."""
        ...
        if qn < 1e-10:
            return -1.0
        mat = np.stack([e.flatten() for e in stored_embeddings])  # (N, D)
        mn = np.linalg.norm(mat, axis=1)  # (N,)
        # avoid div-by-zero
        safe = np.where(mn < 1e-10, 1.0, mn)
        sims = (mat @ q) / (safe * qn)  # (N,)
    ...
    def find_all_matches_independent(self, query_embeddings):
        """
        Independent per-face matching (no greedy one-to-one assignment).

        Each query face independently finds its best registered match, so
        multiple faces in the same frame can match the SAME identity (e.g. a
        real person + a phone photo of that person both resolve to "MG").
        Used for the live preview where we want every face labelled, with
        spoofing handled separately by the liveness layer.

        Returns: list (same length as input) of match dicts or None.
        """
        if not query_embeddings or not self.registered_db:
            return [None] * len(query_embeddings)
        return [
            self.find_match(q) if q is not None else None
