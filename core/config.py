...
class Config:
    ...
    # ──────────────────────────────────────────────
    # Project root (repo root — app runs from here)
    # ──────────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────
    # Face Detector Backend Selection
    # ──────────────────────────────────────────────
    # "yunet"  -> OpenCV YuNet (default; fast, single-stage, pure CPU)
    # "mediapipe" -> MediaPipe Tasks (may crash on some macOS builds)
    # "yolo"   -> YOLOv8-person + Haar cascade fallback (slow)
    ...
    # Hardcoded list of available classes for the attendance dropdown.
    # Edit this list to match your institution's course offerings.
    ...
    # ──────────────────────────────────────────────
    # When a new enrollment's single best embedding-pair similarity to an
    # existing student exceeds this threshold, the registration is rejected
    # as a likely duplicate (same person enrolled twice). Uses max-pair (not
    # averaged) similarity so poor captures don't drag the score down. Same
    # person typically scores 0.95+ on the best pair; genuine twins rarely
    # exceed 0.92.
    BLOCK_DUPLICATE_REGISTRATION: bool = True
    ...
    LIVENESS_IN_PREVIEW: bool = True  # Run quick liveness on every live-preview frame
    ...
    # ──────────────────────────────────────────────
    # Model-Based Anti-Spoofing (MiniFASNetV2)
    # ──────────────────────────────────────────────
    # Minimum "real" probability from the deep-learning spoof model to
    # accept a face as live. Below this → flagged as spoof.
    SPOOF_REAL_THRESHOLD: float = 0.50
