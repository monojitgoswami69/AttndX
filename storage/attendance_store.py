...
class AttendanceStore:
    ...
    def record_check(self, session_id, check_number, detected_student_ids,
                     ...
                     spoofed_student_ids=None):
        """
        Record a check result.

        status: "completed" | "partial" | "failed_dark" | "skipped_dark" | "retried"
        spoofed_student_ids: List of student IDs flagged as spoofed in this check.
                             Spoofed students are NOT counted as present.
        """
        ...
        if spoofed_student_ids is not None:
            check_data["spoofed"] = list(spoofed_student_ids)
        if note:
        ...
        print(
            ...
            f"{f', {len(spoofed_student_ids)} spoofed' if spoofed_student_ids else ''}."
        )
        return True
    ...
    def compute_final(self, session_id, all_student_ids):
        ...
        - Spoofed phases do NOT count toward present.
        - Global status is ALWAYS present or absent:
            >= min_for_present present phases → present
            <  min_for_present present phases → absent
        ...
        # Dynamic threshold (capped by number of valid checks)
        ...
        for student_id in all_student_ids:
            ...
            for cn, cd in valid_checks.items():
                spoofed_ids = cd.get("spoofed", [])
                if student_id in spoofed_ids:
                    # Spoofed phase — does NOT count as present
                    checks_spoofed += 1
                elif student_id in cd.get("detected", []):
                    checks_present += 1
                # else: absent for this phase
            ...
            if checks_spoofed > 0 and not note:
                note = f"Spoofing flagged in {checks_spoofed} phase(s)"

            final_results[student_id] = {
                "checks_present": checks_present,
                "checks_spoofed": checks_spoofed,
                "status": status,
                "note": note,
            }
        ...
        spoofed_n = sum(r.get("checks_spoofed", 0) for r in final_results.values())
        print(
            f"[AttendanceStore] Final ({num_valid} valid checks): "
            f"{present_n} present, {absent_n} absent, {insuf_n} review, "
            f"{spoofed_n} spoofed phase(s) flagged"
        )
        return final_results
